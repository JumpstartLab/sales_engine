  ...
  describe ".most_revenue(x)" do
    ...
    context "when number of items is greater than X" do
      it "returns an array of items with the most revenue" do
        SalesEngine::Database.stub(:items).and_return([item1, item2, item3])
        SalesEngine::Item.most_revenue(2).should == [item3, item2]
      end
    end
    context "when number of items is less than X" do
      it "returns an array of all merchants" do
        SalesEngine::Database.stub(:items).and_return([item1, item2])
        SalesEngine::Item.most_revenue(3).should == [item2, item1]
      end
    end
    context "when there is only one item" do
      it "returns an array with one item" do
        SalesEngine::Database.stub(:items).and_return([item1])
        SalesEngine::Item.most_revenue(3).should == [item1]
      end
    end
    context "when there are no items" do
      it "returns an empty array" do
        SalesEngine::Database.stub(:items).and_return([])
        SalesEngine::Item.most_revenue(3).should == []
      end
    end 
  end
  ...
  describe ".most_revenue(x)" do
    ...
    context "when number of items is greater than X" do
      it "returns an array of items with the most units sold" do
        SalesEngine::Database.stub(:items).and_return([item1, item2, item3])
        SalesEngine::Item.most_items(2).should == [item3, item2]
      end
    end
    context "when number of items is less than X" do
      it "returns an array of all merchants" do
        SalesEngine::Database.stub(:items).and_return([item1, item2])
        SalesEngine::Item.most_items(3).should == [item2, item1]
      end
    end
    context "when there is only one item" do
      it "returns an array with one item" do
        SalesEngine::Database.stub(:items).and_return([item1])
        SalesEngine::Item.most_items(3).should == [item1]
      end
    end
    context "when there are no items" do
      it "returns an empty array" do
        SalesEngine::Database.stub(:items).and_return([])
        SalesEngine::Item.most_items(3).should == []
      end
    end 
  end
